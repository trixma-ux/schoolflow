    ...
    if (studentIds.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            const Icon(Icons.family_restroom, size: 72, color: AppColors.textSecondary),
            const SizedBox(height: 16),
            const Text('Aucun enfant associé à ce compte.', style: TextStyle(color: AppColors.textSecondary, fontSize: 16), textAlign: TextAlign.center),
            const SizedBox(height: 8),
            const Text('Contactez l\'administration pour lier votre enfant.', style: TextStyle(color: AppColors.textSecondary, fontSize: 13), textAlign: TextAlign.center),
          ],
        ),
      );
    }
...
class _ChildOverviewCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    UserModel? student;
    try {
      student = allUsers.firstWhere((u) => u.id == studentId);
    } catch (_) {}
    ...
    if (student?.classId != null) {
      try {
        className = allClasses.firstWhere((c) => c.id == student!.classId).name;
      } catch (_) {}
    }
    ...
    return gradesAsync.when(
      data: (grades) {
        ...
        for (final g in grades) {
          totalScore += (g.score / g.maxScore) * 20;
        }
          ...
          itemBuilder: (context, i) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                onTap: () async {
                  if (!n.isRead) {
                    await CommunicationRepository().markAsRead(n.id);
                  }
                },
              ),
            );
